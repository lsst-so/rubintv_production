...
from lsst.summit.utils.packageVersions import UNKNOWN_VERSION, PackageVersions  # noqa: E402
...
def gather_package_version_comparisons() -> tuple[PackageVersions, list[VersionComparison], str | None]:
    ...
    """
    versions = getCurrentPackageVersions()
...
def print_package_version_summary(
    ...
    if versionHash is not None:
        print(_colour(f"  Version set hash: {versionHash}", _BOLD))

    # rubintv_production can never appear here: it has no Dockerfile ref pin
    # (it is COPYed into the image rather than checked out), so its comparison
    # is always None, never False. The explicit exclusion only exists for the
    # up-front banner in scrape_package_versions.
...
class TestRunner:
    ...
    def __init__(self, run_label: str | None = None) -> None:
        ...
        self.package_version_hash: str | None = None
    ...
    def scrape_package_versions(self) -> None:
        ...
        try:
            ...
            # The content hash is the identity of this version set; it needs no
            # state to compute and is the same for every instrument.
            self.package_version_hash = versions.versionHash()
            print(f"Version set hash: {self.package_version_hash}")
    ...
    def run(self) -> None:
        try:
            ...
            # their Dockerfile agreement and the version set hash. Scary on
            # mismatch, never a failure.
            print_package_version_summary(
                self.package_version_comparisons, self.dockerfile_path, self.package_version_hash
            )
